...
<head>
    ...
    <title>eLMS | View Available Books</title>
...
<body>
    <?php
        ...
        $limit = 4;
        $start = ($page - 1) * $limit;
        $sql = "SELECT books.bid, books.name, books.price, authors.name AS author, categories.name AS category FROM books JOIN authors JOIN categories ON books.author_id = authors.aid AND books.category_id = categories.cid WHERE books.status = 1 LIMIT $start, $limit;";

        $result = $connection -> query("SELECT COUNT(*) AS count FROM books WHERE status = 1;");
    ...
    <div class="form-container">
        <div class="card mt-1">
            <div class="card-header fs-5">
                Total Available Books: <span>
                    <?php echo count_total_available_books();?>
                </span>
            </div>
            <div class="card-body">
                <form class="d-flex my-2" role="search" action="..\user\search_available_book.php" method="post">
                    <input class="form-control me-2" type="text" name="search" placeholder="Search any available book">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination pagination-sm justify-content-end mt-2">
                        <li class="page-item">
                            <a class="page-link"
                                href="..\user\total_available_books.php?page=<?= $previous; ?>">Previous</a>
                        </li>
                        <?php for($i=1; $i<$total_pages; $i++) : ?>
                        <li class="page-item"><a class="page-link"
                                href="..\user\total_available_books.php?page=<?= $i; ?>">
                                <?php echo $i; ?>
                            </a></li>
                        <?php endfor; ?>
                        <li class="page-item">
                            <a class="page-link" href="..\user\total_available_books.php?page=<?= $next; ?>">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
