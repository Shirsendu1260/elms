...
<head>
    ...
    <title>eLMS | Manage Searched Book</title>
    ...
    <div class="form-container">
        <div class="card mt-1">
            ...
            <div class="card-body">
                    ...
                    <table class="table table-sm">
                        <?php
                            if(isset($_POST['submit'])){
                                $search = $_POST['search'];
                                $search_sql = "SELECT books.bid, books.name, books.isbn, books.status, books.locker_no, books.price, authors.name AS author, categories.name AS category FROM books JOIN authors JOIN categories ON books.author_id = authors.aid AND books.category_id = categories.cid WHERE books.name LIKE '%$search%';";
                                ...
                                if($search_sql_run){
                                    if(mysqli_num_rows($search_sql_run) > 0){
                                        ...
                                        $status_text = $status == 1 ? 'Available' : 'Not Available';
                                        ...
                                        $url1 = "../admin/edit_book.php"."?bid=".base64_encode($bid);
                                        $url2 = "../admin/delete_book.php"."?bid=".base64_encode($bid);
                                        echo '<td>'.$locker_no.'</td>
                                                    <td>'.$price.'</td>
                                                    <td>'.$author.'</td>
                                                    <td>'.$category.'</td>
                                                    <td>
                ...
                <a href="../admin/manage_books.php"><button class="btn btn-primary">Back</button></a>
            </div>
        </div>
    </div>
