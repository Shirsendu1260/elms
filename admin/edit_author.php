...
<body>
    <?php
        ...
        $sql = "SELECT * FROM authors WHERE aid = $decoded_aid";
    ...
    <div class="form-container">
        <div class="card col-md-6">
            <div class="card-body">
                <h4 class="card-title my-2 text-center fw-bold">Edit Author</h4>
                <form action="" method="post" class="my-3">
                    <div class="form-group mb-2">
                        <label>ID</label>
                        <input type="number" name="aid" class="form-control" value="<?php echo $aid; ?>" disabled>
                    </div>
                    <div class="form-group mb-2">
                        <label>Author Name</label>
                        <input type="text" name="name" class="form-control required" value="<?php echo $name; ?>">
                    </div>
                    <button type="submit" name="edit_author" class="btn btn-outline-primary my-2">Update</button>
                    <div class="mt-1">
                        <p>Back to <a href="../admin/manage_authors.php" style="text-decoration: none;">Manage Authors</a>.</p>
                    </div>
                </form>
                <?php
                    if(isset($_POST['edit_author'])){
                        $result = $connection -> query("UPDATE authors SET name = '$_POST[name]' WHERE aid = $aid;");
                        if($result){
                            echo '<div id="edit-author-done" style="display: none;"><br><p class="alert alert-success" role="alert">Author edited successfully.</p></div>';
                            echo '<script>
                                setTimeout(function () {
                                    var msg = document.getElementById("edit-author-done");
                            ...
                            echo '<div id="edit-author-not-done" style="display: none;"><br><p class="alert alert-danger" role="alert">Failed to edit author.</p></div>';
                            echo '<script>
                                setTimeout(function () {
                                    var msg = document.getElementById("edit-author-not-done");
                                    msg.style.display = "block";
                                    setTimeout(function () {
                                        msg.style.display = "none";
                                    }, 5000);
